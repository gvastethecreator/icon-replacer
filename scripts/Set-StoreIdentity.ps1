...
if ($null -eq $identityNode -or $null -eq $publisherDisplayNameNode) {
    throw 'Package.appxmanifest is missing Identity or PublisherDisplayName.'
}
...
$storeIdentity.firstStoreArchitecture = 'x64'
...
if (-not $PSCmdlet.ShouldProcess('Icon Replacer manifest, Store identity metadata, and privacy publisher', 'Apply Partner Center identity')) {
    return
}
...
Write-Host 'Icon Replacer Store identity applied.' -ForegroundColor Green
